...
window.addEventListener('scroll', () => {
    ...
    // Show/hide galaxy and stars based on section
    if (points) {
        points.visible = currentSection === 0;  // Show galaxy only in section 0
    }

    if (stars) {
        stars.visible = currentSection !== 0;  // Show stars only in other sections
    }

    // Show/hide GUI based on section
    if (currentSection === 0) {
        gui.domElement.style.display = 'block';  // Show the GUI
    } else {
...
const parameters = {
    ...
    randomness: 0.328,
    randomnessPower: 3,
    insideColor: '#117d92',  // Light blueish, similar to the core of the galaxy
    outsideColor: '#152284', // Very light cyan, like the outer regions of the galaxy
    waveSpeed: 0.4,
    waveHeight: 0.1,
}
